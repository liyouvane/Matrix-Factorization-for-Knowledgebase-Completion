...
#define eta 10
#define GEN 100000
#define gama 0.001//learning rate
...
FILE *fp=fopen("/Users/vanellope/Desktop/毕业论文代码/data/WN18/m3train.dat","r");
FILE *tp=fopen("/Users/vanellope/Desktop/毕业论文代码/data/WN18/m3valid.dat","r");
...
void training(){
    for(long g=1;g<=GEN;g++){
        ...
        for(long i=0;i<num_t;i++){
            ...
            double temp[eta+1];
            if(X[0]<20) {loss-=log(1+exp(X[0]));temp[0]=exp(X[0])/(1+exp(X[0]));}
            else {loss-=X[0];temp[0]=1;}
            v[x[0]]+=gama*temp[0];
            ...
            for(int j=1;j<=eta;j++){
               ...
               // cout<<r1*num_e<<'\n';
                ...
                if(X[j]<20) {loss+=log(1+exp(X[j]))/eta;temp[j]=exp(X[j])/(1+exp(X[j]))/eta;}
                else {loss+=X[j]/eta;temp[j]=1.0/eta;}
                v[x[j]]-=gama*temp[j];
      //          for(int k=0;k<K;k++)
